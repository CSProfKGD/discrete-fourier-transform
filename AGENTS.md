...
- To prevent thumbnail aliasing, show a fixed 64×48 spatial patch anchored at the image origin, as selected by the user. Sample it analytically at the tile's device-pixel resolution, retaining a 4:3 backing grid. Never change the patch extent with frequency. The matched patch/tile aspect ratios preserve stripe orientation; all valid DFT frequencies remain resolved at the minimum tile size. Preserve phase and normalized contrast, with neutral gray only for zero coefficients under current sizing.
...
- Presets are circular in the displayed frequency-bin grid, as explicitly requested: `r=hypot(signedU,signedV)/min(W,H)`. This is a bin-space radius, not an isotropic cycles-per-pixel radius on a rectangular image. Low is `L(.055)`, where `L(s)=exp(-r²/(2s²))`. High-pass has zero transmission through .065, a raised-cosine transition to .14, and full transmission beyond .14, making its central stop band visible in log magnitude. Band-pass has zero below .035 and above .18, full transmission .055–.14, raised-cosine tapers across .035–.055 and .14–.18. Modify coefficients, never fake an annulus with display styling.
